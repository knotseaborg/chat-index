...
class Message(Base):
    ...
    previous_links = relationship(
        "Link", back_populates="next_message", foreign_keys="Link.next_message_id"
    )
    ...
    summaries_ending_here = relationship(
        "Summary", back_populates="end_message", foreign_keys="Summary.end_message_id"
    )
...
class Summary(Base):
    ...
    end_message = relationship(
        "Message", back_populates="summaries_ending_here", foreign_keys=[end_message_id]
    )
...
class Link(Base):
    ...
    previous_message = relationship(
        "Message", back_populates="next_links", foreign_keys=[previous_message_id]
    )
    next_message = relationship(
        "Message", back_populates="previous_links", foreign_keys=[next_message_id]
    )
